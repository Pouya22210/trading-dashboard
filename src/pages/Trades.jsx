...
// v11.4: Day of week definitions for filter
const DAYS_OF_WEEK = [
...
// v11.4: Day of week filter (all days selected by default)
...
// v11.4: Day of week toggle helpers
...
// v11.4: Quick preset to exclude Fridays
const excludeFridays = useCallback(() => {
  setSelectedDays(DAYS_OF_WEEK.filter(d => d.key !== 5).map(d => d.key))
}, [])
...
useEffect(() => {
setCurrentPage(1)
}, [filters, selectedChannelIds, selectedDays])
...
const filteredTrades = useMemo(() => {
    return trades.filter(trade => {
      ...
      // v11.4: Day of week filter
      if (selectedDays.length < 7 && trade.signal_time) {
      ...
      }, [trades, selectedChannelIds, filters, showOrphanedChannels, selectedDays])
...
<div>
  <label className="flex items-center gap-2 text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
    <Calendar className="w-3 h-3 text-accent-cyan" /> Day of Week
  </label>
  <div className="bg-dark-tertiary/50 rounded-lg p-3">
        ...
        <button
          ...
          className={`px-2.5 py-1.5 rounded text-xs font-medium transition-all ${
            selectedDays.includes(day.key)
              ...
              : 'bg-dark-secondary text-gray-500 border border-dark-border hover:border-gray-500'
          }`}
          title={day.fullLabel}
        >
          {day.label}
    ...
    {selectedDays.length < 7 && (
      <p className="text-xs text-gray-500 mt-2">
        Showing {selectedDays.length} of 7 days
        {!selectedDays.includes(5) && ' (Fridays excluded)'}
      </p>
    )}
  </div>
</div>
